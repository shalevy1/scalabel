...
import { Label2DSpecType, State } from '../../functional/types'
...
export function makeDrawableLabel2D (
  labelType: string,
  labelSpecs: { [name: string]: Label2DSpecType }
): Label2D | null {
  if (labelType in labelSpecs) {
    return new CustomLabel2D(labelSpecs[labelType])
  }
  switch (labelType) {
...
export class Label2DList {
  ...
  /** label specs */
  private _labelSpecs: { [name: string]: Label2DSpecType }
  /** callbacks */
  ...
  public updateState (state: State): void {
    this._state = state
    this._labelSpecs = state.task.config.label2DSpecs
    const self = this
    ...
    _.forEach(item.labels, (label, key) => {
      ...
      if (!(labelId in self._labels)) {
        const newLabel = makeDrawableLabel2D(label.type, this._labelSpecs)
        if (newLabel) {
